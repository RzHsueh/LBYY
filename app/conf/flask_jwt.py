...
def jwt_required(fn):
    ...
    def wapper(*args, **kwargs):
        ...
        if not auth_header_value:
            return jsonify(code='2100', msg='Authorization missing')

        parts = auth_header_value.split()
        if len(parts) == 1:
            return jsonify(code='2100', msg='Token missing')  # code 仅作为示例

        elif len(parts) > 2:
            return jsonify(code='2100', msg='Token is invalid')

        token = parts[1]
        if token is None:
            return jsonify(code='2100', msg='Token exception')
        ...
        if payload.get('identity') is None:
            return jsonify(code='2100', msg='user does not exist')
